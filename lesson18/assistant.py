...
Бот помічник повинен стати для нас прототипом додатка-асистента. Додаток-асистент в першому
наближенні повинен уміти працювати з книгою контактів і календарем. У цій домашній роботі
зосередимося на інтерфейсі самого бота. Найбільш простий і зручний на початковому етапі
розробки інтерфейс - це консольний додаток CLI (Command Line Interface). CLI досить просто
реалізувати. Будь-який CLI складається з трьох основних елементів:

1) Парсер команд. Частина, яка відповідає за розбір введених користувачем рядків,
виділення з рядка ключових слів та модифікаторів команд.
2) Функції обробники команд — набір функцій, які ще називають handler, вони відповідають
за безпосереднє виконання команд.
3) Цикл запит-відповідь. Ця частина програми відповідає за отримання від користувача даних
...
що виникають у функціях-handler (KeyError, ValueError, IndexError) та повертати відповідну
...
COMMANDS = {'hello': hello, 'add': add_contact, 'change': change_contact, 'show all': show_all,
            'phone': show_phone, 'exit': goodbye, 'close': goodbye, 'stop': goodbye, '.': goodbye,
            'help': help_me, '?': help_me}


# add Oleksandr +380937316049
# Oleksandr +380937316049
def main():
    contacts = {}
    print('Enter the command or ? for help')
    while True:
        user_input = input('Enter command: ')
        for key in COMMANDS.keys():
            if user_input.lower().startswith(key):
                args = user_input[len(key):].split()  # ['Oleksandr', '+380937316049']
                result = COMMANDS[key](contacts, *args)  # функція хендлер
                if result is None:
                    print('Good bye!')
                    exit(0)
                print(result, '\n')
